...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    for item in rawFeats:
        features.append(OHEDict[item])
    n = len(features)
    feats = [1.0] * n
    features.sort()
    return SparseVector(numOHEFeats, features, feats)
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    Args:
        point (str): A comma separated string where the first value is the label and the rest
            are features. 
            feats: list of (featureID, value) tuples
    ...
    sparse = oneHotEncoding(feats, OHEDict, numOHEFeats)
...
classOneFracTrain = OHETrainData.map(lambda x: x.label).sum()/float(OHETrainData.count())
...
def evaluateResults(model, data):
    ...
    """
    probs =  data.map(lambda x: (getP(x.features, model.weights, model.intercept),x.label)).map(lambda (x,y): computeLogLoss(x,y)).sum()/float(data.count())
...
# TODO: Replace <FILL IN> with appropriate code
logLossValBase = OHEValidationData.map(lambda x: computeLogLoss(classOneFracTrain,x.label)).sum()/float(OHEValidationData.count())
...
def parseHashPoint(point, numBuckets):
    ...
    feats2 = hashFunction(numBuckets, feats)
    
    
    
    return LabeledPoint(label, SparseVector(numBuckets, feats2))
...
def computeSparsity(data, d, n):
    ...
    """
    sumratios = float(data.map(lambda lp: float(len(lp.features.indices))).sum())/float(d)

    return float(sumratios)/float(n) 
...
logLossTestBaseline = hashTestData.map(lambda x: computeLogLoss(classOneFracTrain,x.label)).sum()/float(hashTestData.count())
