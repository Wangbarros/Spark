...
def parsePoint(line):
    ...
    """
    intermed = line.split(",")
    point = LabeledPoint(intermed[0],intermed[1:])
...
print parsedSamplePoints.take(1)[0].label
firstPointFeatures = parsedSamplePoints.map(lambda x: x.features).take(1)[0]
firstPointLabel = parsedSamplePoints.map(lambda x: x.label).take(1)[0]
...
averageTrainYear = (parsedTrainData
                    .map(lambda x: x.label).sum())/float(parsedTrainData.count())
...
def calcRMSE(labelsAndPreds):
    ...
    """
    SqError = labelsAndPreds.map(lambda (x,y): squaredError(x,y))
    RMSE = (SqError.sum()/float(SqError.count()))**(0.5)
...
labelsAndPreds = parsedTrainData.map(lambda x: getLabeledPrediction(weightsLR0,x))
...
def twoWayInteractions(lp):
    ...
    """
    lista = []
    cartesian =  list(itertools.product(lp.features,lp.features))
    for item in cartesian:
        item = np.prod(item)
        lista.append(item)
    return LabeledPoint(lp.label,list(np.hstack((lp.features,lista))))
